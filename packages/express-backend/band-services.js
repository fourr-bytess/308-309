...
function getBandsPaginated(limit, offset, filters = {}) {
  ...
  const bandsPromise = bandModel
    .find(query)
    .skip(offset)
    .limit(limit)
    .select(BAND_SELECT);
  const countPromise = bandModel.countDocuments(query);
  return Promise.all([bandsPromise, countPromise]).then(([bands, total]) => ({
    bands,
    total,
  }));
}
...
function addBandMember(bandId, musicianId) {
  return bandModel.findByIdAndUpdate(
    bandId,
    { $addToSet: { members: musicianId } },
    { new: true },
  );
}

function removeBandMember(bandId, musicianId) {
  return bandModel.findByIdAndUpdate(
    bandId,
    { $pull: { members: musicianId } },
    { new: true },
  );
}
